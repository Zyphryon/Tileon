...
namespace Tileon::Editor
{
    ...
    Glaze::Glaze(Ref<Context> Context)
        : mContext  { Context },
    ...
    Bool Glaze::Create(Text Path, AnyRef<Content::Uri> Key)
    {
    ...
    void Glaze::Open(Text Path, AnyRef<Content::Uri> Key)
    {
        mPath = Str(Path);
        mKey  = Move(Key);
        ...
        {
            LOG_E("Glaze: failed to read '{0}'", mPath);
            return;
        }
        ...
        {
            LOG_E("Glaze: '{0}' is not a material", mPath);
            return;
        }
    ...
    Bool Glaze::Draw()
    {
        ...
        if (Toolkit::Composer::Begin("Material##Editor", Visible))
        {
            ...
            for (UInt32 Index = 0; Index < mBindings.GetSize(); ++Index)
            {
                ...
                const UInt64 Key = HashCombine("Vault.Material.Path", Index);

                if (Str Selection; mSelector.Consume(Key, Selection))
                {
    ...
    Bool Glaze::Write(Text Path, ConstSpan<Binding> Bindings, ConstSpan<Constant> Constants)
    {
        JsonValue Document;
        Document.SetObject();
        ...
        {
            LOG_E("Glaze: failed to write '{0}'", Path);
            return false;
        }
        return true;
    }
