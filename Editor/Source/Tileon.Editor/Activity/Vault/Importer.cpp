...
namespace Tileon::Editor
{
    ...
    void Importer::Browse(Kind Kind)
    {
        ...
        mPicker.Open(Toolkit::Picker::Mode::Open, Filesystem::GetRootFolder(), Filter, [this, Kind](Text Path)
        {
            mImport  = Str(Path);
            mPending = Kind;
        });
    ...
    Bool Importer::ImportFont(Text Source, Text Folder)
    {
        ...
        const ::Pipeline::Baker::Font::Baker Assembler(mContext.GetScheduler());

        if (!Assembler.Bake(Source, Baked, Settings))
        {
    ...
    Bool Importer::ImportTexture(Text Source, Text Folder)
    {
        ...
        const ::Pipeline::Baker::Texture::Baker Assembler(mContext.GetScheduler());
        ...
        if (Normal)
        {
            ...
            Assembler.Bake(Sibling, Str::Print<"{0}/{1}_n.tex">(Folder, Stem), Settings);
        }
    ...
    {
        // The editor authors materials through one writer, so an imported one is shaped like an authored one.
        Sequence<Glaze::Binding> Bindings;

        Glaze::Binding Albedo;
        Albedo.Name = "Albedo";
        Albedo.Path = Str::Print<"{0}.tex">(Stem);
        Bindings.Append(Move(Albedo));

        if (Normal)
        {
            Glaze::Binding Direction;
            Direction.Name = "Normal";
            Direction.Path = Str::Print<"{0}_n.tex">(Stem);
            Bindings.Append(Move(Direction));
        }

        Glaze::Write(Str::Print<"{0}/{1}.mtl">(Folder, Stem), Bindings);
    }
}