...
namespace Tileon::Editor
{
    ...
    void Archetypes::DrawDetailsPanel()
    {
        ...
        mAssembler.Draw(mSelection.GetEntity());
    }
    ...
    void Archetypes::DrawPreviewPanel()
    {
        ...
        {
            if (ConstRetainer<Graphic::Image> Albedo = Material->GetImage(Hash(Enum::GetName(Texture::Albedo))))
            {
                if (Toolkit::Composer::BeginTabItem("Preview"))
                {
                    ...
                    mPreviewer.Draw(Albedo->GetHandle(), Size, Source, Tint ? Color::FromColor8(* Tint) : Color::White());
            ...
            {
                if (ConstRetainer<Graphic::Image> Texture = Material->GetImage(Hash(Enum::GetName(Semantic))))
                {
    ...
    void Archetypes::DrawEmptyPanel(Text Message)
    {
