...
namespace Tileon::Editor
{
    ...
    static Bool InspectSunDirection(Text Label, Ref<Vector3> Value)
    {
        ...
        List->AddCircleFilled(Center, Radius, Toolkit::Theme::kFieldFace);
        List->AddCircle(Center, Radius, Toolkit::Theme::kFieldEdge);
        List->AddLine(ImVec2(Center.x - Radius, Center.y), ImVec2(Center.x + Radius, Center.y), Toolkit::Theme::kFieldGrid);
        List->AddLine(ImVec2(Center.x, Center.y - Radius), ImVec2(Center.x, Center.y + Radius), Toolkit::Theme::kFieldGrid);

        const Bool Above = (Sun.GetY() >= 0.0f);

        List->AddLine(Center, Handle, Toolkit::Theme::kActiveSoft);
        List->AddCircleFilled(Handle, 5.0f, Above ? Toolkit::Theme::kActive : Toolkit::Theme::kFieldHollow);
        List->AddCircle(Handle, 5.0f, Toolkit::Theme::kActive);
        ...
        AnyRef<Callback>       Action)
    {
        ...
        if (Str Selection; Browser.Consume(Key, Selection))
        {
    ...
    {
        Animation::Flipbook Flipbook = Component.GetFlipbook();
        Bool                Dirty    = false;

        Toolkit::Composer::TextDisabled("{0} frame(s), {1:.2f}s total", Flipbook.GetCount(), Flipbook.GetDuration());
        Toolkit::Composer::Spacing();

        if (Toolkit::Composer::DisabledButton(ICON_FA_PLUS "  Add Frame", Flipbook.IsFull(), -1.0f))
        {
            Flipbook.Insert(0.1f, Rect(0.0f, 0.0f, 1.0f, 1.0f));
            Dirty = true;
        }
        ...
        if (Toolkit::Composer::BeginTable("##frames", 7, kTableFlags))
        {
            ...
            for (UInt8 Keyframe = 0; Keyframe < Flipbook.GetCount(); ++Keyframe)
            {
                const Rect Data     = Flipbook.GetData(Keyframe);
                Real32     X        = Data.GetMinimumX();
                Real32     Y        = Data.GetMinimumY();
                Real32     Width    = Data.GetWidth();
                Real32     Height   = Data.GetHeight();
                Real32     Duration = Flipbook.GetDuration(Keyframe);
                ...
                if (Enclosure)
                {
                    Flipbook.SetData(Keyframe, Rect(X, Y, X + Width, Y + Height));
                    Dirty = true;
                }

                if (OnCell(5, "##t", Duration, "%.2f"))
                {
                    Flipbook.SetDuration(Keyframe, Max(Duration, 0.01f));
                    Dirty = true;
                }
            ...
            if (Discard < Flipbook.GetCount())
            {
                Flipbook.Remove(Discard);
                Dirty = true;
            }
        }
        ...
        if (Dirty)
        {
            Component.SetFlipbook(Move(Flipbook));
        }
        return Dirty;
    }
    ...
    Bool Inspect(Ref<Workspace> Workspace, Scene::Entity Actor, Ref<Decoration> Component)
    {
        ...
        if (Dirty)
        {
            const Render::FontEffect Values(
                OutsetColor,
                OutsetOffset,
                OutsetWidth,
                OutsetBias,
