...
namespace Tileon::Editor::Toolkit
{
    ...
    void Theme::Initialize(Ref<Content::Service> Content)
    {
        // The embedded mount borrows its bytes from static storage, so the atlas must never take ownership of them.
        ...
        Symbols.PixelSnapH           = true;
        Symbols.GlyphMinAdvanceX     = 13.0f;
        ...
        const auto OnLoadFont = [&](Text Path, ConstPtr<ImFontConfig> Config, ConstPtr<ImWchar> Range)
        {
            Content.Read(Path, [&](Filesystem::Result Result, Blob Data)
            {
                if (Result != Filesystem::Result::Success || Data.GetSize() == 0)
                {
                    LOG_E("Theme: failed to read embedded font '{0}'", Path);
                    return;
                }
                Atlas->AddFontFromMemoryTTF(Data.GetData(), static_cast<SInt32>(Data.GetSize()), 16.0f, Config, Range);
            });
        };

        OnLoadFont("Embedded://Font/Roboto-Medium.ttf", &Regular, nullptr);
        OnLoadFont("Embedded://Font/FontAwesome-Solid.ttf", &Symbols, kIconRange);
