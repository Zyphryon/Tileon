...
namespace Tileon::Editor::Toolkit
{
    ...
    void Picker::Travel(SInt32 Steps)
    {
    ...
    void Picker::Ascend()
    {
        ...
        if (const Text Parent = StrBeforeLast(Trimmed, '/'); !Parent.IsEmpty())
        {
            ...
            Navigate(StrEndsWith(Parent, ":")
                ? Text(Filesystem::Path::Join(Parent, '/', Text::Empty()))
                : Parent);
        }
    }
    ...
    Bool Picker::Locate(Text Address)
    {
    ...
    void Picker::DrawToolbar()
    {
        ...
        if (mEditing)
        {
            ...
            Toolkit::Composer::InputText("##picker_address", mAddress, [this](Text Value)
            {
                mAddress = Value;
        ...
        else
        {
            ...
            Toolkit::Composer::BeginChild("##picker_trail", ImVec2(Width, Toolkit::Composer::GetFrameHeight()));
        ...
        Toolkit::Composer::InputText("##picker_search", mSearch, [this](Text Value)
        {
            mSearch = Value;
        });
    }
    ...
    void Picker::DrawTrail(Real32 Width)
    {
        ...
        {
            if (Toolkit::Composer::InvisibleButton("##picker_address_hit",
    ...
    Bool Picker::DrawListing()
    {
        ...
        // settling its own size, which hid the first rows on the frame the dialog opened.
        ...
        if (Toolkit::Composer::BeginTable("##picker_list", 3, kTableFlags, ImVec2(0.0f, Height)))
        {
            ...
            for (UInt Index = 0, Limit = mEntries.GetSize(); Index < Limit; ++Index)
            {
                ...
                if (Clicked)
                {
                    ...
                    // A double click opens what was hit: folders step in, files confirm the dialog.
    ...
    Bool Picker::DrawFooter()
    {
        ...
        if (Toolkit::Composer::BeginCombo("##picker_filter", Filter))
        {
    ...
    void Picker::Draw()
    {
        ...
        // The dialog lays itself out to the window, so it never scrolls as a whole; only the listing does.
        constexpr ImGuiWindowFlags kWindowFlags =
            ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse;

        if (Toolkit::Composer::BeginPopupModal("##picker", kWindowFlags))
        {
        ...
        else if (!Toolkit::Composer::IsPopupOpen("##picker"))
        {
            // The popup was dismissed from outside, so drop the dialog instead of reopening it next frame.
            mOpen = false;
        }
    }
}
