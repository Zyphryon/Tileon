...
namespace Tileon::Editor
{
    ...
    Resources::Resources(Ref<Context> Context)
        : Panel         { Context, "Resources", true },
          mCreating     { Creation::None },
          mImporter     { Context },
          mMaterializer { Context }
    {
        Refresh();
    }
    ...
    Resources::Kind Resources::Classify(Text Name)
    {
    ...
    Text Resources::GetIcon(Kind Kind)
    {
        switch (Kind)
        {
        case Kind::Folder:
    ...
    void Resources::Navigate(AnyRef<Str> Folder)
    {
        mFolder    = Move(Folder);
        mSelection = Str();
    ...
    Str Resources::Resolve(Text Name)
    {
    ...
    Content::Uri Resources::Locate(Text Name)
    {
    ...
    void Resources::DrawEntryMenu(ConstRef<Entry> Item)
    {
        ...
        {
            mMaterializer.Open(Resolve(Item.Name), Locate(Item.Name));
        }
    ...
    void Resources::DrawFolderPrompt()
    {
        ...
        if (Toolkit::Composer::Button("Create", 96.0f))
        {
            if (!mCreation.IsEmpty())
            {
                ...
                else
                {
                    ...
                    if (mMaterializer.Create(Resolve(Name), Locate(Name)))
                    {
                        Refresh();
                    }
                }
    ...
    void Resources::Reload(ConstRef<Entry> Item)
    {
