// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// Copyright (C) 2025-2026 Tileon contributors (see AUTHORS.md)
//
// This work is licensed under the terms of the MIT license.
//
// For a copy, see <https://opensource.org/licenses/MIT>.
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// [  HEADER  ]
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

#include "Launcher.hpp"
#include "Tileon.Editor/Toolkit/Theme.hpp"

// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// [   CODE   ]
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

namespace Tileon::Editor
{
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    static Bool AccentButton(Text Label, Real32 Width, Real32 Height = 0.0f)
    {
        Toolkit::Composer::PushStyleColor(ImGuiCol_Button,        ImVec4(0.18f, 0.40f, 0.62f, 1.00f));
        Toolkit::Composer::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.24f, 0.50f, 0.74f, 1.00f));
        Toolkit::Composer::PushStyleColor(ImGuiCol_ButtonActive,  ImVec4(0.28f, 0.56f, 0.82f, 1.00f));
        const Bool Pressed = Toolkit::Composer::Button(Label, Width, Height);
        Toolkit::Composer::PopStyleColor(3);
        return Pressed;
    }

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    Launcher::Launcher()
        : mState { State::Menu }
    {
        LoadRecent();
    }

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    Launcher::Result Launcher::Draw()
    {
        if (mState == State::Done)
        {
            return Result::Done;
        }

        // Draw a solid background to cover the entire display area.
        Toolkit::Composer::GetBackgroundDrawList()->AddRectFilled(ImVec2(0, 0),
            Toolkit::Composer::GetDisplaySize(), Toolkit::Theme::kBackdrop);

        switch (mState)
        {
        case State::Menu:
            Toolkit::Composer::SetNextWindowSize(300.0f, 380.0f);
            break;
        case State::Wizard:
            Toolkit::Composer::SetNextWindowSize(460.0f, 320.0f);
            break;
        default:
            break;
        }
        Toolkit::Composer::SetNextWindowPos(Toolkit::Composer::GetViewportCenter(), ImGuiCond_Always, ImVec2(0.5f, 0.5f));

        constexpr ImGuiWindowFlags kWindowFlags =
            ImGuiWindowFlags_NoResize              |
            ImGuiWindowFlags_NoMove                |
            ImGuiWindowFlags_NoCollapse            |
            ImGuiWindowFlags_NoScrollbar           |
            ImGuiWindowFlags_NoNavFocus            |
            ImGuiWindowFlags_NoBringToFrontOnFocus |
            ImGuiWindowFlags_NoDocking             |
            ImGuiWindowFlags_NoTitleBar            |
            ImGuiWindowFlags_NoDecoration;

        Result Result = Result::None;

        if (Toolkit::Composer::Begin("##bootstrap", kWindowFlags))
        {
            Toolkit::Composer::Spacing();

            switch (mState)
            {
            case State::Menu:
                Result = DrawInMenu();
                break;
            case State::Wizard:
                Result = DrawInWizard();
                break;
            default:
                break;
            }

            Toolkit::Composer::End();
        }

        mExplorer.Draw();

        return Result;
    }

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    Launcher::Result Launcher::DrawInMenu()
    {
        const Real32 Width = Toolkit::Composer::GetContentRegionAvail().x;

        Toolkit::Composer::SetWindowFontScale(2.4f);
        Toolkit::Composer::CenterCursor("Welcome");
        Toolkit::Composer::TextColored(ImVec4(0.46f, 0.72f, 0.98f, 1.00f), "Welcome");
        Toolkit::Composer::SetWindowFontScale(1.0f);

        Toolkit::Composer::Spacing();
        Toolkit::Composer::Separator();
        Toolkit::Composer::Spacing();

        constexpr Real32 kHeight = 38.0f;

        if (AccentButton(ICON_FA_FOLDER_PLUS "   Create New", Width, kHeight))
        {
            mState = State::Wizard;
        }

        Toolkit::Composer::Spacing();

        if (Toolkit::Composer::Button(ICON_FA_FOLDER_OPEN "   Open Project", Width, kHeight))
        {
            BrowseToOpen();
        }

        // Recent projects, most-recently opened first.
        if (!mRecent.IsEmpty())
        {
            Toolkit::Composer::Spacing();
            Toolkit::Composer::Header("Recent");
            Toolkit::Composer::Spacing();

            SInt32 Clicked = -1;

            for (UInt32 Index = 0; Index < mRecent.GetSize(); ++Index)
            {
                ConstRef<Str> Path  = mRecent[Index];
                const Text    Label = StrAfterLast(Path, '/');

                if (Toolkit::Composer::Selectable(String<288>::Print<"{0}##recent_{1}">(Label, Index), false))
                {
                    Clicked = static_cast<SInt32>(Index);
                }
                Toolkit::Composer::Tooltip(Path);
            }

            if (Clicked >= 0)
            {
                const Str Path = mRecent[Clicked];
                OpenRecent(Path);
            }
        }

        Toolkit::Composer::SetCursorPosY(Toolkit::Composer::GetWindowBottom(2));
        Toolkit::Composer::Separator();
        Toolkit::Composer::Spacing();

#if !defined(ZY_PLATFORM_WEB)
        if (Toolkit::Composer::Button(ICON_FA_RIGHT_FROM_BRACKET "   Exit", Width, kHeight))
        {
            return Result::Exit;
        }
#endif

        return Result::None;
    }

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    Launcher::Result Launcher::DrawInWizard()
    {
        constexpr Real32 kLabelW = 110.0f;

        Toolkit::Composer::SetWindowFontScale(1.5f);
        Toolkit::Composer::TextColored(ImVec4(0.46f, 0.72f, 0.98f, 1.00f), ICON_FA_CUBES "  New Project");
        Toolkit::Composer::SetWindowFontScale(1.0f);

        Toolkit::Composer::Spacing();
        Toolkit::Composer::Separator();
        Toolkit::Composer::Spacing();

        Toolkit::Composer::Label(ICON_FA_PEN_TO_SQUARE "  Name");
        Toolkit::Composer::SameLine(kLabelW);
        Toolkit::Composer::SetNextItemWidth(-1.0f);
        Toolkit::Composer::InputText("##name", mProject.GetName(), [this](Text Value)
        {
            mProject.SetName(Value);
        });

        Toolkit::Composer::Spacing();

        constexpr Array<UInt16, 6> kDensityOptions = { 8, 16, 32, 64, 128, 256 };

        Toolkit::Composer::Label(ICON_FA_LAYER_GROUP "  Density");
        Toolkit::Composer::SameLine(kLabelW);
        Toolkit::Composer::SetNextItemWidth(-1.0f);
        if (Toolkit::Composer::BeginCombo("##density", String<32>::Print<"{0} px">(mProject.GetDensity())))
        {
            for (const UInt16 Option : kDensityOptions)
            {
                const Bool Selected = (mProject.GetDensity() == Option);

                if (Toolkit::Composer::Selectable(String<32>::Print<"{0} px">(Option), Selected))
                {
                    mProject.SetDensity(Option);
                }
            }
            Toolkit::Composer::EndCombo();
        }

        Toolkit::Composer::Spacing();

        Toolkit::Composer::Label(ICON_FA_ALIGN_LEFT "  Description");
        Toolkit::Composer::SameLine(kLabelW);
        Toolkit::Composer::SetNextItemWidth(-1.0f);
        Toolkit::Composer::InputText("##description", mProject.GetDescription(), [this](Text Value)
        {
            mProject.SetDescription(Value);
        });

        Toolkit::Composer::SetCursorPosY(Toolkit::Composer::GetWindowBottom(2));
        Toolkit::Composer::Separator();
        Toolkit::Composer::Spacing();

        constexpr Real32 kButtonH = 34.0f;
        const Real32     kButtonW = (Toolkit::Composer::GetContentRegionAvail().x - Toolkit::Composer::GetStyle().ItemSpacing.x) * 0.5f;

        if (Toolkit::Composer::Button(ICON_FA_CHEVRON_LEFT "  Back", kButtonW, kButtonH))
        {
            mState = State::Menu;
        }

        Toolkit::Composer::SameLine();

        if (AccentButton(ICON_FA_CHECK "  Create", kButtonW, kButtonH))
        {
            BrowseToSave();
        }

        return Result::None;
    }

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    void Launcher::BrowseToOpen()
    {
        mExplorer.Open(Toolkit::Explorer::Mode::Open, Filesystem::GetRootFolder(), Project::kFileExtension,
            [this](Text Path)
            {
                OnBrowseResult(Path);
            });
    }

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    void Launcher::BrowseToSave()
    {
        mExplorer.Open(Toolkit::Explorer::Mode::Save, Filesystem::GetRootFolder(), Project::kFileExtension,
            [this](Text Path)
            {
                OnBrowseResult(Path);
            });
    }

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    void Launcher::OnBrowseResult(Text Path)
    {
        Str Result(Path);

        // Ensure the selected file has the correct project file extension, appending it if necessary.
        if (!StrEndsWith(Result, Project::kFileExtension))
        {
            Result.Append('.');
            Result.Append(Project::kFileExtension);
        }
        mProject.SetPath(Move(Result));

        switch (mState)
        {
        case State::Menu:
        {
            Blob File;

            if (Filesystem::Read(mProject.GetPath(), File) == Filesystem::Result::Success)
            {
                JsonValue Parser = JsonDocument::Parse(Text(File.GetData<Char>(), File.GetSize()));

                if (mProject.Load(Parser))
                {
                    PushRecent(mProject.GetPath());

                    mState = State::Done;
                }
            }
            break;
        }
        case State::Wizard:
        {
            JsonValue Parser;
            Parser.SetObject();
            mProject.Save(Parser);

            if (Scaffold(StrBeforeLast(mProject.GetPath(), '/')))
            {
                Filesystem::Write(mProject.GetPath(), JsonDocument::Dump(Parser));

                PushRecent(mProject.GetPath());

                mState = State::Done;
            }
            else
            {
                LOG_W("Launcher: Failed to prepare project directory at '{}'", mProject.GetPath());

                Filesystem::Delete(mProject.GetPath());
            }
            break;
        }
        default:
            break;
        }
    }

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    Bool Launcher::Scaffold(Text Path)
    {
        Filesystem::Path Source = Filesystem::GetRootFolder();
        Source.Append("Launcher/");

        if (Filesystem::CopyAll(Source, Path) == Filesystem::Result::Success)
        {
            Filesystem::Make(Path + Filesystem::Path("/Data"));
            Filesystem::Make(Path + Filesystem::Path("/Font"));
            Filesystem::Make(Path + Filesystem::Path("/Material"));
            Filesystem::Make(Path + Filesystem::Path("/World"));
            return true;
        }
        return false;
    }

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    void Launcher::OpenRecent(Text Path)
    {
        mProject.SetPath(Str(Path));

        Blob File;

        if (Filesystem::Read(mProject.GetPath(), File) == Filesystem::Result::Success)
        {
            JsonValue Parser = JsonDocument::Parse(Text(File.GetData<Char>(), File.GetSize()));

            if (mProject.Load(Parser))
            {
                PushRecent(mProject.GetPath());

                mState = State::Done;
                return;
            }
        }

        // The file is gone or unreadable, so drop it from the list to stop it reappearing.
        mRecent.RemoveIf([&](ConstRef<Str> Existing)
        {
            return Existing == Path;
        });
        SaveRecent();
    }

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    void Launcher::LoadRecent()
    {
        mRecent.Clear();

        Blob File;

        if (Filesystem::Read(Filesystem::GetDataFolder("Tileon", "Editor") + "Recent.json", File) != Filesystem::Result::Success)
        {
            return;
        }

        JsonValue Document = JsonDocument::Parse(Text(File.GetData<Char>(), File.GetSize()));

        if (!Document.IsObject())
        {
            return;
        }

        const JsonObject Root(Document);
        const JsonArray  Items = Root.GetArray("Recent");

        if (Items.IsNull())
        {
            return;
        }

        for (UInt Index = 0; Index < Items.GetSize() && mRecent.GetSize() < kMaxRecent; ++Index)
        {
            if (const Text Path = Items.GetString(Index); !Path.IsEmpty())
            {
                mRecent.Append(Path);
            }
        }
    }

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    void Launcher::SaveRecent()
    {
        JsonValue Document;
        Document.SetObject();

        JsonObject Root(Document);
        JsonArray  Items = Root.SetArray("Recent");

        for (ConstRef<Str> Path : mRecent)
        {
            Items.AddString(Path);
        }

        Filesystem::Write(Filesystem::GetDataFolder("Tileon", "Editor") + "Recent.json", JsonDocument::Dump(Document));
    }

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    void Launcher::PushRecent(Text Path)
    {
        // Move the path to the front (most recent), de-duplicating any earlier appearance.
        mRecent.RemoveIf([&](ConstRef<Str> Existing)
        {
            return Existing == Path;
        });
        mRecent.Insert(0, Path);

        // Drop the oldest entries once the cap is exceeded.
        while (mRecent.GetSize() > kMaxRecent)
        {
            mRecent.RemoveLast();
        }
        SaveRecent();
    }
}