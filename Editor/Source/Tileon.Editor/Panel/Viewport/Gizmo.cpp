...
namespace Tileon::Editor
{
    ...
    Bool Gizmo::Draw(ConstRef<Bag<UInt64>> Selection, Scene::Entity Primary, ImVec2 Origin, ImVec2 Size)
    {
        ...
        const Camera   Camera(mContext.GetDirector(), Origin, Size);
        const ImVec2 Anchor = Camera.Project(GetOrigin(Primary), Primary.Get<const Pose>().GetTranslation().GetY());
        ...
        {
        case Mode::Move:
            Hovered = DrawMove(Camera, Anchor);
            break;
        case Mode::Rotate:
            Hovered = DrawRotate(Camera, Anchor);
            break;
        case Mode::Scale:
            Hovered = DrawScale(Camera, Anchor);
            break;
        }
        ...
        if (mHandle == Handle::None)
        {
            ...
            {
                mHandle    = Hovered;
                mGrab      = Camera.Unproject(Toolkit::Composer::GetMousePos());
        ...
        const Placement Cursor = Camera.Unproject(Toolkit::Composer::GetMousePos());
        const Vector2   Delta(
        ...
        const ImVec2 Screen = GetAxisOffset(Camera, mHandle);
    ...
    ImVec2 Gizmo::GetAxisOffset(ConstRef<Camera> Camera, Handle Handle)
    {
        const Vector3 Axis = GetAxis(Handle);
        return Axis.IsAlmostZero() ? ImVec2(0.0f, 0.0f) : Camera.Direction(Axis);
    }

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    Gizmo::Handle Gizmo::DrawMove(ConstRef<Camera> Camera, ImVec2 Anchor) const
    {
        ...
        const ImVec2 EndX = AxisTip(Anchor,   GetAxisOffset(Camera, Handle::AxisX), kAxisLength);
        const ImVec2 EndZ = AxisTip(Anchor,   GetAxisOffset(Camera, Handle::AxisZ), kAxisLength);
        const ImVec2 EndY = AxisTip(LiftBase, GetAxisOffset(Camera, Handle::AxisY), kAxisLength);
        ...
        DrawArrow(List, Anchor,   EndX, Engaged == Handle::AxisX ? Toolkit::Theme::kActive : Toolkit::Theme::kAxisX);
        DrawArrow(List, Anchor,   EndZ, Engaged == Handle::AxisZ ? Toolkit::Theme::kActive : Toolkit::Theme::kAxisZ);
        DrawArrow(List, LiftBase, EndY, Engaged == Handle::AxisY ? Toolkit::Theme::kActive : Toolkit::Theme::kAxisY);

        List->AddCircleFilled(Anchor, kPickRadius, Engaged == Handle::Plane ? Toolkit::Theme::kActive : Toolkit::Theme::kAxisFree);
        return Hovered;
    }
    ...
    Gizmo::Handle Gizmo::DrawRotate(ConstRef<Camera> Camera, ImVec2 Anchor) const
    {
        const Ptr<ImDrawList> List   = Toolkit::Composer::GetWindowDrawList();
        const ImVec2          Cursor = Toolkit::Composer::GetMousePos();
        ...
        {
            { .Ring = Handle::RingZ, .First = Handle::AxisX, .Second = Handle::AxisY, .Color = Toolkit::Theme::kAxisZ },
            { .Ring = Handle::RingY, .First = Handle::AxisX, .Second = Handle::AxisZ, .Color = Toolkit::Theme::kAxisY },
            { .Ring = Handle::RingX, .First = Handle::AxisY, .Second = Handle::AxisZ, .Color = Toolkit::Theme::kAxisX },
        };
        ...
        {
            const ImVec2 First  = GetAxisOffset(Camera, kRings[Index].First);
            const ImVec2 Second = GetAxisOffset(Camera, kRings[Index].Second);
        ...
        for (UInt32 Index = 0; Index < 3; ++Index)
        {
            ...
            List->AddPolyline(
                Points[Index], kSegments, Active ? Toolkit::Theme::kActive : kRings[Index].Color,
        ...
        if (Engaged != Handle::None)
        {
            ...
            if (Reach > 0.0001f)
            {
                ...
                List->AddCircleFilled(Knob, 5.0f, Toolkit::Theme::kActive);
                List->AddLine(Anchor, Knob, Toolkit::Theme::kActive, 1.0f);
            }
        }
        return Hovered;
    }
    ...
    Gizmo::Handle Gizmo::DrawScale(ConstRef<Camera> Camera, ImVec2 Anchor) const
    {
        const Ptr<ImDrawList> List   = Toolkit::Composer::GetWindowDrawList();
        const ImVec2          Cursor = Toolkit::Composer::GetMousePos();
        ...
        {
            const UInt32 Color = (Engaged == Which) ? Toolkit::Theme::kActive : Tone;
        ...
        DrawStub(Anchor,   EndX, Handle::AxisX, Toolkit::Theme::kAxisX);
        DrawStub(LiftBase, EndY, Handle::AxisY, Toolkit::Theme::kAxisY);
        DrawStub(Anchor,   EndZ, Handle::AxisZ, Toolkit::Theme::kAxisZ);

        // Drawn as a box rather than the move handle's circle, so the uniform grab reads as scaling at a glance.
        const UInt32     Color  = (Engaged == Handle::Uniform) ? Toolkit::Theme::kActive : Toolkit::Theme::kAxisFree;
