...
namespace Tileon
{
    ...
    public:
        ...
        ZY_INLINE void SetTile(UInt8 X, UInt8 Y, AnyRef<Tile> Tile)
        {
            mTiles[ConvertTo1D(X, Y, kTilesPerX)] = Move(Tile);
        }
        ...
        ZY_INLINE void Erase(IntRect Area, Tile::Layer Layer)
        {
            for (UInt8 Y = Area.GetMinimumY(); Y < Area.GetMaximumY(); ++Y)
            {
                for (UInt8 X = Area.GetMinimumX(); X < Area.GetMaximumX(); ++X)
        ...
        ZY_INLINE void Clear(IntRect Area)
        {
            for (UInt8 Y = Area.GetMinimumY(); Y < Area.GetMaximumY(); ++Y)
            {
                for (UInt8 X = Area.GetMinimumX(); X < Area.GetMaximumX(); ++X)
